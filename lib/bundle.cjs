...
class ProgressManager {
  ...
  updateProgress(processed, operation) {
    ...
    const output = this.isVerbose ? `\r${operation}: ${percentStr}% (${processed}/${this.totalItems}) Rate: ${rateStr}/sec ETA: ${etaStr} Elapsed: ${this.formatTime(elapsed / 1000)}` : `\r${operation}: ${percentStr}% (${processed}/${this.totalItems}) ETA: ${etaStr}`;
...
Interrupted. Saving progress...`);
  ...
  async saveProgressState() {
    try {
      const stateFile = import_path.join(import_os.tmpdir(), "ocsight-progress.json");
...
async function findOpenCodeDataDirectory(customPath) {
  ...
  try {
    await import_promises2.stat(basePath).catch(async () => {
      await import_promises2.mkdir(basePath, { recursive: true });
    });
    return basePath;
  } catch (error) {
...
function calculateFileHash(content) {
  return import_crypto.createHash("sha256").update(content).digest("hex").substring(0, HASH_LENGTH);
}
async function getFileMetadata(filePath) {
  const file2 = runtime.file(filePath);
  const content = await file2.text();
...
function handleError(error, context, filePath) {
  if (error instanceof DataProcessingError) {
    if (error.recoverable) {
      console.warn(`Recoverable error in ${context}: ${error.message}`, filePath ? `File: ${filePath}` : "");
      return;
    }
    throw error;
  }
  if (error instanceof Error) {
    console.error(`Unexpected error in ${context}: ${error.message}`, error.stack, filePath ? `File: ${filePath}` : "");
    throw new DataProcessingError(`Unexpected error: ${error.message}`, filePath, context, false);
  }
  console.error(`Unknown error in ${context}:`, error);
  throw new DataProcessingError("Unknown error occurred", filePath, context, false);
}
  ...
  for (const [sessionId, sessionMeta] of sessionMap) {
    ...
    const processedMessages = messages.map((msg) => {
      ...
      if (typeof msg.content === "string") {
        const codeBlockMatches = msg.content.match(/```(?:bash|tool|sh|shell|command)\s*\n([\s\S]*?)\n```/g);
        if (codeBlockMatches) {
        ...
        const directToolMentions = msg.content.match(/\b(?:bash|read|write|glob|grep|edit|list|task|chrome|figma|serena|vibe|context7|webfetch|sequential-thinking)\b/gi);
        ...
        const executionPatterns = msg.content.match(/(?:executing|running|using|called)\s*[:\-]?\s*([a-zA-Z][a-zA-Z0-9_-]*)/gi);
      ...
      if (msg.system && Array.isArray(msg.system)) {
        ...
        const toolDefinitions = systemText.match(/(?:function|tool|method)\s+[a-zA-Z][a-zA-Z0-9_-]*\s*\([^)]*\)/gi);
        if (toolDefinitions) {
          toolDefinitions.forEach((def) => {
            const toolName = def.match(/(?:function|tool|method)\s+([a-zA-Z][a-zA-Z0-9_-]*)/i)?.[1];
            if (toolName) {
              tools.push({
                name: toolName.toLowerCase(),
                duration_ms: 0,
        ...
        const toolLists = systemText.match(/(?:available|supported)\s+tools?\s*[:\-]?\s*([^.\n]+)/gi);
        if (toolLists) {
        ...
        const toolPatterns = systemText.match(/(?:bash|read|write|glob|grep|edit|list|task|chrome|figma|serena|vibe|context7|webfetch|sequential-thinking)\s*\([^)]*\)/gi);
        if (toolPatterns) {
          toolPatterns.forEach((pattern) => {
            const toolName = pattern.match(/(bash|read|write|glob|grep|edit|list|task|chrome|figma|serena|vibe|context7|webfetch|sequential-thinking)/i)?.[1] || "unknown";
            tools.push({
              name: toolName.toLowerCase(),
              duration_ms: 0,
              timestamp: new Date(msg.time.created).toISOString()
...
async function loadOpenCodeData(options) {
  ...
  const progressManager = new ProgressManager(Math.max(sessionFiles.length, messageFiles.length), { quiet: options?.quiet, verbose: options?.verbose });
  ...
  if (options?.days) {
    ...
    for (const file2 of messageFiles) {
      try {
        const fileObj = runtime.file(file2);
  ...
  if (useCache && cache) {
    ...
    if (!options?.quiet) {
      console.log(`Processing ${filesToProcess.length} new/changed files, ${unchangedFiles.length} cached files`);
    }
  } else {
    filesToProcess.push(...limitedMessageFiles);
    if (!options?.quiet) {
      console.log(`Processing ${filesToProcess.length} message files${options?.days ? ` (filtered to ${options.days} days)` : ""} (cache disabled or unavailable)`);
    }
  }
  ...
  const sessionPromises = sessionFiles.map(async (file2) => {
    return await safeFileOperation(async () => {
      ...
      if (!validateSessionData(data)) {
        throw new DataProcessingError("Invalid session data structure", file2, "session validation", true);
      }
      return data;
    }, file2, "session loading");
  });
  ...
  if (sessionLoadErrors > 0 && !options?.quiet) {
    console.warn(`Failed to load ${sessionLoadErrors}/${sessionFiles.length} session files`);
  }
  ...
  for (let i = 0;i < allMessageFiles.length; i += BATCH_SIZE) {
    ...
    const batchPromises = batch.map(async (file2) => {
      return await safeFileOperation(async () => {
        ...
        if (!validateMessageData(data)) {
          throw new DataProcessingError("Invalid message data structure", file2, "message validation", true);
        }
        const message = data;
    ...
    if (i % PROGRESS_UPDATE_INTERVAL === 0 && i > 0 && !options?.quiet) {
      console.log(`Processed ${Math.min(i, allMessageFiles.length)}/${allMessageFiles.length} message files...`);
    }
  }
  if (!options?.quiet) {
    const totalMessages = Array.from(messagesBySession.values()).reduce((total, msgs) => total + msgs.length, 0);
  ...
  for await (const sessionData of streamProcessSessions(sessionMap, messagesBySession)) {
...
var PathsConfig = exports_external.object({
  data_dir: exports_external.string().default("~/.local/share/opencode/storage/message"),
...
class SessionManager {
  ...
  async buildIndex(quiet = false) {
    ...
    for (const filePath of files) {
      ...
      const stats = await import_promises9.stat(fullPath).catch(() => null);
      if (!stats)
        continue;
  ...
  startWatcher(onChange) {
    ...
    (async () => {
      try {
        const watcher = import_promises8.watch(messageDir, {
          recursive: true,
...
//# debugId=F64CCEC4428ED66864756E2164756E21
