...
async function findMostRecentlyActiveSession() {
  try {
    ...
    const messagePath = (0, import_path6.join)(dataPath, "storage", "message");
    const entries = await (0, import_promises6.readdir)(messagePath, { withFileTypes: true });
    ...
    for (const sessionId of sessionDirs) {
      try {
        const sessionPath = (0, import_path6.join)(messagePath, sessionId);
        const messageFiles = await (0, import_promises6.readdir)(sessionPath);
        for (const messageFile of messageFiles) {
          if (!messageFile.endsWith(".json")) continue;
          const messagePath2 = (0, import_path6.join)(sessionPath, messageFile);
          const stats = await (0, import_promises6.stat)(messagePath2);
...
async function calculateCurrentContextTokens(messageFiles, messagePath) {
  try {
    ...
    for (const file of messageFiles) {
      try {
        const filePath = (0, import_path6.join)(messagePath, file);
    ...
    for (const { file } of messageData) {
      try {
        const messageFilePath = (0, import_path6.join)(messagePath, file);
        const content = await (0, import_promises6.readFile)(messageFilePath, "utf8");
...
async function getActiveSessionInfo(sessionId) {
  try {
    const dataPath = getDefaultOpenCodePath();
    const messagePath = (0, import_path6.join)(dataPath, "storage", "message", sessionId);
    const messageFiles = await (0, import_promises6.readdir)(messagePath);
    ...
    for (const messageFile of jsonFiles) {
      try {
        const messageFilePath = (0, import_path6.join)(messagePath, messageFile);
        const stats = await (0, import_promises6.stat)(messageFilePath);
        ...
        if (message.role === "assistant" && message.providerID && message.modelID) {
          const messageTime = message.time?.completed || message.time?.created || stats.mtime.getTime();
    ...
    try {
      const { findModel: findModel2, calculateModelCost: calculateModelCost2 } = await Promise.resolve().then(() => (init_models_db(), models_db_exports));
      const modelId = `${provider}/${model}`;
      const modelData = await findModel2(modelId);
      if (modelData) {
        ...
        realCost = calculateModelCost2(modelData, tokenDistribution);
      }
    } catch (error) {
    }
    return {
...
var LIVE_MONITORING_LIMIT = 5e4;
var BURN_RATE_HIGH_THRESHOLD = 5e4;
...
var RECENT_SESSIONS_DISPLAY = 5;
var DEBUG_SESSION_LIMIT = 50;
var DEBUG_DAYS_BACK = 1;

// packages/cli/src/lib/live-ui.ts
var BURN_RATE_NORMAL_THRESHOLD = 2e4;
...
function formatElapsedTime(startTime = SESSION_START_TIME) {
...
var LiveMonitor = class {
  ...
  async renderDashboard(status, options) {
    ...
    const sections = [];
    const elapsedTime = status.recentActivity ? formatElapsedTime(
    ...
    sections.push({
      title: "SESSION",
      content: [
        source_default.white("Project: ") + source_default.cyan(projectName) + source_default.white("  ID: ") + source_default.cyan(status.sessionId) + source_default.white("  Started: ") + source_default.cyan(sessionStartTime.toLocaleTimeString()) + source_default.white("  Elapsed: ") + source_default.cyan(elapsedTime)
      ],
    ...
    const burnText = burnRateInfo.status === "No activity" ? "IDLE" : `${burnRateInfo.status} ${burnRateInfo.text}`;
    const modelInfo = status.currentModel ? status.currentModel.split("/").slice(-1)[0] : "unknown";
    sections.push({
      title: "USAGE",
      content: [
        source_default.white("Model: ") + source_default.cyan(modelInfo) + source_default.white("  Tokens: ") + source_default.bold.cyan(tokensText) + source_default.white("  Cost: ") + source_default.bold.green(costText) + source_default.white("  Burn: ") + burnRateInfo.color(burnText)
      ],
      color: source_default.green
    });
...
var liveCommand = new Command("live").description("Monitor OpenCode usage in real-time").option("--config <path>", "Path to config file").option(
  ...
  try {
    ...
    if (options.session) {
      console.log(
        source_default.dim(`Monitoring specific session: ${options.session}`)
      );
    } else {
    ...
    try {
      ...
      if (debugData.sessions.length > 0) {
        const recentSessions = debugData.sessions.sort(
    ...
    const getStatus = async () => {
      try {
        ...
        if (!data.sessions.length) {
          ...
          const directSessionId = await findMostRecentlyActiveSession2();
          if (directSessionId) {
            const directInfo = await getActiveSessionInfo3(directSessionId);
            if (directInfo) {
              const { findModel: findModel3 } = await Promise.resolve().then(() => (init_models_db(), models_db_exports));
              const modelInfo2 = await findModel3(
                `${directInfo.provider}/${directInfo.model}`
              );
              const contextLimit2 = modelInfo2?.limit?.context || 128e3;
              return {
        ...
        if (options.session) {
          ...
          activeSession = data.sessions.find(
            (s) => s.id === sessionIdToFind || s.id.startsWith(sessionIdToFind)
          );
          if (!activeSession) {
            console.error(
              source_default.red(
            ...
            const AVAILABLE_SESSIONS_DISPLAY2 = 5;
            data.sessions.slice(0, AVAILABLE_SESSIONS_DISPLAY2).forEach((s) => {
              console.log(
        ...
        const ACTIVITY_WINDOW_MINUTES2 = 5;
        const BURN_RATE_SMOOTHING_MINUTES2 = 5;
        const MIN_MESSAGES_FOR_RATE2 = 1;
        const MS_TO_MINUTES = 1e3 * 60;
        let burnRate = 0;
        if (realSessionInfo && realSessionInfo.messageCount > MIN_MESSAGES_FOR_RATE2) {
          const minutesAgo = (Date.now() - realSessionInfo.lastActivity.getTime()) / MS_TO_MINUTES;
          if (minutesAgo < ACTIVITY_WINDOW_MINUTES2) {
            const avgTokensPerMessage = realSessionInfo.totalTokens.total / realSessionInfo.messageCount;
            burnRate = Math.floor(
              avgTokensPerMessage / BURN_RATE_SMOOTHING_MINUTES2
            );
          }
        }
        const FALLBACK_CONTEXT_RATIO2 = 0.01;
        let sessionTokenData;
        let currentContextTokens = Math.floor(
          activeSession.tokens_used * FALLBACK_CONTEXT_RATIO2
        );
        if (realSessionInfo) {
          sessionTokenData = {
        ...
        const mockSessionData = {
          tokens: sessionTokenData || {
            input: Math.floor(
              activeSession.tokens_used * TOKEN_ESTIMATES2.INPUT_RATIO
            ),
            output: Math.floor(
              activeSession.tokens_used * TOKEN_ESTIMATES2.OUTPUT_RATIO
            ),
            reasoning: Math.floor(
              activeSession.tokens_used * TOKEN_ESTIMATES2.REASONING_RATIO
            ),
            cache: {
              write: Math.floor(
                activeSession.tokens_used * TOKEN_ESTIMATES2.CACHE_WRITE_RATIO
              ),
              read: Math.floor(
                activeSession.tokens_used * TOKEN_ESTIMATES2.CACHE_READ_RATIO
              )
            }
          },
        ...
        const modelTotals = await (async () => {
          ...
          if (modelSessions.length === 0) return void 0;
          const { calculateModelCost: calculateModelCost2 } = await Promise.resolve().then(() => (init_models_db(), models_db_exports));
          let totalTokens = 0;
          let totalRealCost = 0;
          for (const session of modelSessions) {
            ...
            const tokenBreakdown = {
              input: Math.floor(
                (session.tokens_used || 0) * TOKEN_ESTIMATES2.INPUT_RATIO
              ),
              output: Math.floor(
                (session.tokens_used || 0) * TOKEN_ESTIMATES2.OUTPUT_RATIO
              ),
              reasoning: Math.floor(
                (session.tokens_used || 0) * TOKEN_ESTIMATES2.REASONING_RATIO
              ),
              cache_write: Math.floor(
                (session.tokens_used || 0) * TOKEN_ESTIMATES2.CACHE_WRITE_RATIO
              ),
              cache_read: Math.floor(
                (session.tokens_used || 0) * TOKEN_ESTIMATES2.CACHE_READ_RATIO
              )
            };
            if (modelInfo) {
              const sessionRealCost = calculateModelCost2(
                modelInfo,
                tokenBreakdown
              );
        ...
        return {
          ...
          recentActivity: realSessionInfo ? {
            tokens: Math.floor(
              realSessionInfo.totalTokens.total / realSessionInfo.messageCount
            ),
            // Avg per message
