...
var SessionManager = class {
  ...
  async buildIndex(quiet = false) {
    ...
    const files = await (0, import_glob.glob)("**/ses_*.json", { cwd: sessionDir });
    for (const filePath of files) {
      const fullPath = (0, import_path6.join)(sessionDir, filePath);
      const stats = await (0, import_promises7.stat)(fullPath).catch(() => null);
      if (!stats) continue;
  ...
  async loadMessagesForSession(sessionId) {
    ...
    const messages = [];
    try {
      const files = await (0, import_glob.glob)("msg_*.json", { cwd: sessionMessageDir });
      for (const filePath of files) {
        const fullPath = (0, import_path6.join)(sessionMessageDir, filePath);
        const content = await (0, import_promises7.readFile)(fullPath, "utf-8").catch(() => null);
        if (!content) continue;
  ...
  startWatcher(onChange) {
    ...
    (async () => {
      try {
        const watcher = (0, import_promises6.watch)(messageDir, {
          recursive: true,
...
budgetCommand.command("add").description("Add budget limit for a provider").option("--config <path>", "Path to config file").action(async (options) => {
  try {
    ...
    await (0, import_promises8.writeFile)(configPath, JSON.stringify(fullConfig, null, 2), "utf8");
    console.log(
      statusIndicator(
        "success",
...
var getVersion = async () => {
  ...
  try {
    ...
    const packageJson = JSON.parse(await (0, import_promises9.readFile)(packageJsonPath, "utf-8"));
