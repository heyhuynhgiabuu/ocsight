...
export function getDefaultOpenCodePath() {
    const platform = process.platform;
    if (platform === "win32") {
        return join(process.env.USERPROFILE || homedir(), ".local", "share", "opencode");
    }
    // macOS and Linux
    return join(homedir(), ".local", "share", "opencode");
}
export async function findOpenCodeDataDirectory(customPath) {
...
async function hasFileChanged(filePath, cacheEntry) {
    ...
    return (metadata.mtime !== cacheEntry.mtime ||
...
export async function findSessionFiles(dataDir) {
    const files = [];
...
export async function findMessageFiles(dataDir) {
    const files = [];
...
export async function loadOpenCodeData(options) {
    ...
    if (useCache && cache) {
        ...
        console.log(`Processing ${filesToProcess.length} message files (cache disabled or unavailable)`);
    }
    // Load session metadata with enhanced error handling
    const sessionMap = new Map();
    let sessionLoadErrors = 0;
    for (const file of sessionFiles) {
        const session = await safeFileOperation(async () => {
            const content = await readFile(file, "utf-8");
            const data = JSON.parse(content);
            if (!validateSessionData(data)) {
                throw new DataProcessingError("Invalid session data structure", file, "session validation", true);
            }
            return data;
        ...
        const batchPromises = batch.map(async (file) => {
            return await safeFileOperation(async () => {
                ...
                if (!validateMessageData(data)) {
                    throw new DataProcessingError("Invalid message data structure", file, "message validation", true);
                }
                const message = data;
