...
async function calculateCurrentContextTokens(messageFiles: string[], messagePath: string): Promise<number> {
  try {
...
export async function getActiveSessionInfo(sessionId: string): Promise<ActiveSessionInfo | null> {
  try {
    const dataPath = getDefaultOpenCodePath();
    ...
    for (const messageFile of jsonFiles) {
      try {
        ...
        // Get model info from assistant messages
    ...
    totalTokens.total = totalTokens.input + totalTokens.output + totalTokens.reasoning + 
    ...
    try {
      const { calculateRealMessageCost } = await import('./quota-utils.js');
      const modelId = `${provider}/${model}`;
      realCost = await calculateRealMessageCost(totalTokens, modelId);
    } catch (error) {
