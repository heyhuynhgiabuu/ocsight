...
export const formatTokenBreakdown = (
  ...
  if (tokens.cache_read > 0 || tokens.cache_write > 0) {
    const cacheEfficiency = tokens.cache_read > 0 
    ...
    if (tokens.cache_read > tokens.cache_write * 2) {
      cacheLines.push(chalk.green("💡 Excellent cache utilization - saving costs!"));
    } else if (tokens.cache_write > tokens.cache_read) {
      cacheLines.push(chalk.yellow("⚠️  High cache write ratio - consider optimizing"));
    }
